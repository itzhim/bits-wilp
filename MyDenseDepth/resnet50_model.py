...
def create_model(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        if is_twohundred:
            ...
            base_model = applications.ResNet50(input_shape=(None, None, 3), include_top=False)
        ...
        decoder = upproject(decoder, int(decode_filters/2), 'up1', concat_with='add_13')
        decoder = upproject(decoder, int(decode_filters/4), 'up2', concat_with='add_10')
        decoder = upproject(decoder, int(decode_filters/8), 'up3', concat_with='activation_2')
        decoder = upproject(decoder, int(decode_filters/16), 'up4', concat_with='max_pooling2d')
