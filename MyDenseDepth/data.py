...
def get_nyu_data(batch_size, nyu_data_zipfile='nyu_dataset/nyu_dataset.zip'):
...
class NYU_BasicAugmentRGBSequence(Sequence):
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        ...
        for i in range(batch_x.shape[0]):
            ...
            y = np.clip(np.asarray(Image.open( "nyu_dataset/"+sample[1] )).reshape(480,640,1)/255*self.maxDepth,0,self.maxDepth)
